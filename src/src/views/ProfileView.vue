      ...
      <div class="profile-card">
        ...
        <div class="user-info">
          ...
          <div class="user-meta">
            ...
            <span @click="activeTab = 'following'" class="clickable-meta">关注：{{ user.following_count }}</span>
            <span @click="activeTab = 'followers'" class="clickable-meta">粉丝：{{ user.follower_count }}</span>
          </div>
    ...
    <div class="tab-content">
      ...
      <div v-if="activeTab === 'following'" class="follow-section">
        ...
        <div v-else class="follow-list">
          <div v-for="followedUser in followingList" :key="followedUser.id" class="follow-item" @click="goToUserProfile(followedUser.id)">
      ...
      <div v-if="activeTab === 'followers'" class="follow-section">
        ...
        <div v-else class="follow-list">
          <div v-for="followerUser in followersList" :key="followerUser.id" class="follow-item" @click="goToUserProfile(followerUser.id)">
        ...
        <form @submit.prevent="saveProfile">
          <div class="avatar-container" @click="triggerEditAvatarUpload">
            <div class="avatar-wrapper">
              ...
              <input type="file" ref="editAvatarInput" @change="handleEditAvatarChange" hidden accept="image/*" />
            </div>
          </div>
          ...
          <hr class="password-divider"> <h4>修改密码 (可选)</h4>
          ...
          <div class="form-group">
              ...
              />
              <p v-if="editErrors.currentPassword" class="error-message">{{ editErrors.currentPassword }}</p>
          </div>
           <div class="form-group">
           ...
           <div class="form-group">
              ...
              <input
                  ...
                  autocomplete="new-password"
              />
              <p v-if="editErrors.confirmNewPassword" class="error-message">{{ editErrors.confirmNewPassword }}</p>
          </div>
...
onMounted(async () => {
  ...
  // If the URL has a query param indicating a tab, switch to it
    ...
    if (tabFromQuery && ['posts', 'following', 'followers', 'likes', 'collections'].includes(tabFromQuery)) {
      ...
      if (tabFromQuery === 'following' && followingList.value.length === 0 && !loadingFollowing.value) fetchFollowing();
      if (tabFromQuery === 'followers' && followersList.value.length === 0 && !loadingFollowers.value) fetchFollowers();
      // Add similar logic for likes/collections if they need initial fetch
    }
})
...
watch(activeTab, (newTab, oldTab) => {
    ...
    if (newTab === 'following' && followingList.value.length === 0 && !loadingFollowing.value) {
        fetchFollowing();
    } else if (newTab === 'followers' && followersList.value.length === 0 && !loadingFollowers.value) {
        fetchFollowers();
    }
    // Add watch logic for likes/collections here
});
...
async function fetchProfileAndInitialData(token: string) {
      ...
      if (profileRes.data.state === 1 && profileRes.data.data) {
        ...
        console.error('Failed to fetch profile:', profileRes.data.message)
         if (profileRes.data.code === 401) { // Check for specific unauthorized code if backend provides it
              alert("认证失败，请重新登录。");
              router.push('/login');
         } else {
      ...
      if (currentUserId !== null) {
        try {
          const followStatsRes = await axios.get(`http://127.0.0.1:5000/follow-count/${currentUserId}`, {
          ...
          if (followStatsRes.data && typeof followStatsRes.data.following_count !== 'undefined') {
            ...
            console.error('Failed to fetch follow stats or data in unexpected format:', followStatsRes.data);
...
function validateEditForm(): boolean {
    ...
    // Validate Email (optional format check)
     ...
     // Validate Phone (optional format check)
      if (editForm.phone.trim()) { // Phone is optional, validate only if entered
    ...
    if (editForm.newPassword || editForm.confirmNewPassword) {
        ...
        if (!editForm.newPassword) {
            ...
            isValid = false;
        } else if (editForm.newPassword.length < 6) { // Example: minimum 6 characters
        ...
        if (editForm.newPassword && editForm.confirmNewPassword && editForm.newPassword !== editForm.confirmNewPassword) {
...
async function saveProfile() {
    ...
    if (editForm.avatarFile) {
        ...
        console.log("Appending avatar file to FormData."); // 调试用
    } else {
        console.log("No new avatar file selected."); // 调试用
    }
    try {
        ...
        if (res.data.state === 1) {
            ...
            if (res.data.code === 400 && res.data.message === 'Incorrect current password') { // Example backend error message
...
// --- Follow/Follower List Specific Logic (Keep existing) ---
                 ...
                 if (followedUser.id) {
                     ...
                     followingUsersWithStatus.push({ ...followedUser, ...status, statusLoading: false });
                      ...
                      // Push with minimal info if ID is missing
                       followingUsersWithStatus.push({ id: followedUser.id || -1, username: followedUser.username || 'Unknown', avatar: followedUser.avatar || defaultAvatar, intro: followedUser.intro || 'No intro', statusLoading: false });
                 }
            }
                  ...
                  if (followerUser.id) {
                     const status = await fetchFollowStatus(followerUser.id);
                      ...
                      console.warn("Follower user object is missing ID:", followerUser);
                       // Push with minimal info if ID is missing
                       followerUsersWithStatus.push({ id: followerUser.id || -1, username: followerUser.username || 'Unknown', avatar: followerUser.avatar || defaultAvatar, intro: followerUser.intro || 'No intro', statusLoading: false });
                  }
            }
...
async function fetchFollowStatus(targetUserId: number): Promise<{ is_following?: boolean, is_followed_by?: boolean, is_mutual?: boolean }> {
    ...
    try {
        ...
        if (res.data && typeof res.data.is_following !== 'undefined') {
            ...
            console.warn(`Follow status for user ${targetUserId} data is not in expected format:`, res.data);
         ...
         if (isAxiosError(error)) {
            if (error.response?.status === 404) {
                console.warn(`User ${targetUserId} not found when fetching follow status.`);
                 ...
                 console.error(`API error fetching follow status for ${targetUserId}:`, error.response?.data?.message || error.message);
            }
        } else {
...
async function handleFollowButtonClick(targetUser: UserWithStatus) {
    ...
    const targetUserInList = followingList.value.find(u => u.id === targetUser.id) || followersList.value.find(u => u.id === targetUser.id);
    if (!targetUserInList) {
...
async function followUser(targetUser: UserWithStatus) {
    ...
    try {
        ...
        const res = await axios.post(`http://127.0.0.1:5000/follow/${targetUser.id}`, {}, {
            headers: { Authorization: `Bearer ${token}` }
        });

        if (res.data.state === 1) { // Assuming success state is 1
             ...
             if (user.value.id !== null) { // Ensure current user ID is available
                 const followStatsRes = await axios.get(`http://127.0.0.1:5000/follow-count/${user.value.id}`, {
                      headers: { Authorization: `Bearer ${token}` } // Assuming follow-count might need auth now
                 });
...
@media (max-width: 600px) {
  ...
  .bio { font-size: 14px; }
  .user-meta { gap: 5px 10px; margin-bottom: 15px; }
  .user-meta span { font-size: 13px; padding: 4px 8px; }
  ...
  .post-section, .follow-section, .placeholder-section { padding: 15px; }
   .post-section h3, .follow-section h3, .placeholder-section h3 { font-size: 20px; margin-bottom: 20px;}
